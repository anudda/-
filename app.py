...
# [추가] 영상을 페이지 내 자동 재생하기 위해 데이터를 읽어오는 함수
...
st.markdown("""
    ...
    .info-box {
        ...
        padding: 25px;
        border-radius: 20px;
        box-shadow: 2px 5px 15px rgba(233, 30, 99, 0.1);
        text-align: center;
        margin-bottom: 25px;
...
# 4. 동영상 출력 (페이지 내 자동 재생 + 화질 보존)
try:
    video_data = get_video_base64("baby.mp4")
    st.markdown(f"""
        <div style="text-align: center;">
            <video width="100%" autoplay loop muted playsinline style="border-radius: 20px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);">
                <source src="data:video/mp4;base64,{video_data}" type="video/mp4">
            </video>
        </div>
    """, unsafe_allow_html=True)
except Exception as e:
    st.error("영상을 불러오는데 실패했습니다. 파일명을 확인해 주세요!")
